...
describe('WikiProjectBuilder Component - Update Mode', () => {
  describe('when the user is logged in', () => {
    ...
    describe('and the builder is found', () => {
      beforeEach(() => {
        cy.intercept('GET', '**/v1/builders/3', { fixture: 'wikiproject_builder.json' }).as('builder');
        
        cy.mount(WikiProjectBuilder, {
          route: '/selections/wikiproject/3',
          routes: [
      ...
      it('displays a textbox with invalid project names', () => {
        cy.intercept('POST', '**/v1/builders/3', { fixture: 'save_wikiproject_failure.json' }).as('save');
      ...
      it('sends correct data to API', () => {
        cy.intercept('POST', '**/v1/builders/3', { fixture: 'save_list_success.json' }).as('updateBuilderSuccess');
        cy.get('#updateListButton').click();
        cy.wait('@updateBuilderSuccess').then((interception) => {
          ...
          expect(interception.request.body.params.exclude).to.deep.equal(['Countries']);
        });
      });

      it('redirects on saving valid project names', () => {
        cy.intercept('POST', '**/v1/builders/3', { fixture: 'save_list_success.json' }).as('save');
        cy.get('#updateListButton').click();
    ...
    describe('and the builder has fatal errors', () => {
      beforeEach(() => {
        cy.intercept('GET', '**/v1/builders/3', { fixture: 'wikiproject_builder_fatal_error.json' }).as('builder');
    ...
    describe('and the builder has retryable errors', () => {
      beforeEach(() => {
        cy.intercept('GET', '**/v1/builders/3', { fixture: 'wikiproject_builder_retryable_error.json' }).as('builder');
    ...
    describe('and the builder is not found', () => {
      beforeEach(() => {
        cy.intercept('GET', '**/v1/builders/3', { statusCode: 404, body: '404 NOT FOUND' }).as('builder');
