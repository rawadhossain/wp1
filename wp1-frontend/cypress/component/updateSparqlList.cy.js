...
describe('SparqlBuilder Component - Update Mode', () => {
  describe('when the user is logged in', () => {
    ...
    describe('and the builder is found', () => {
      beforeEach(() => {
        cy.intercept('GET', '**/v1/builders/2', { fixture: 'sparql_builder.json' }).as('builder');
        
        cy.mount(SparqlBuilder, {
          route: '/selections/sparql/2',
          routes: [
      ...
      it('displays builder information', () => {
        ...
        cy.get('#items > .form-control').should('have.value', 'SELECT ?foo WHERE {}');
      ...
      it('displays server validation errors', () => {
        ...
        cy.intercept('POST', '**/v1/builders/2', { fixture: 'save_sparql_failure.json' }).as('save');
      ...
      it('saves successfully after fixing invalid query', () => {
        cy.intercept('POST', '**/v1/builders/2', (req) => {
          if (req.body.params.query.indexOf('WHERE') === -1) {
            req.reply({ statusCode: 200, fixture: 'save_sparql_failure.json' });
          } else {
            req.reply({ statusCode: 200, fixture: 'save_sparql_success.json' });
          }
        }).as('save');
        ...
        cy.get('#items > .form-control').click().clear().type('SELECT ?foo WHERE {}', { parseSpecialCharSequences: false });
        cy.get('#updateListButton').click();
      ...
      describe('on update success', () => {
        beforeEach(() => {
          cy.intercept('POST', '**/v1/builders/2', { fixture: 'save_list_success.json' }).as('save');
        });
    ...
    describe('and the builder has fatal errors', () => {
      beforeEach(() => {
        cy.intercept('GET', '**/v1/builders/2', { fixture: 'sparql_builder_fatal_error.json' }).as('builder');
    ...
    describe('and the builder has retryable errors', () => {
      beforeEach(() => {
        cy.intercept('GET', '**/v1/builders/2', { fixture: 'sparql_builder_retryable_error.json' }).as('builder');
    ...
    describe('and the builder is not found', () => {
      beforeEach(() => {
        cy.intercept('GET', '**/v1/builders/2', { statusCode: 404, body: '404 NOT FOUND' }).as('builder');
